...
\end{aligned}
$$
若 $\mathbf{Y}=\mathbf{X}$，则称之为方差，简记为 $\Sigma=\operatorname{Cov}[\mathbf{X}]$。易知协方差矩阵*对称且半正定*。
...
$$
故
$$\mathbb{E}\left[\boldsymbol{X}\boldsymbol{Y}^\mathsf{T}\right]=\Sigma+\mu\mu^\mathsf{T}$$

## 仿射变换后的方差

$$\mathrm{Cov}\left[A\boldsymbol{X}+b,C\boldsymbol{Y}+d\right]=\mathbb{E}\left[(\boldsymbol{X}-\mathbb{E}\left[\boldsymbol{X}\right])(\boldsymbol{Y}-\mathbb{E}\left[\boldsymbol{Y}\right])^\mathsf{T}\right]$$
