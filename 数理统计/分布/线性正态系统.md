...
由[[正态分布的条件分布]]可知，其后验分布 $\mathbf{X|Y}$ 也服从正态分布，记为 $\mathcal{N}(\boldsymbol{\mu}_{x|y},\boldsymbol{\Sigma}_{x|y})$。代入得
$$\begin{aligned}
...
由其联合分布可知，$Y$ 的边际分布为正态分布，均值为 $\mathbf{W}\boldsymbol{\mu}_x+\boldsymbol{b}$，方差为 $\boldsymbol{\Sigma}_y+\mathbf{W}\boldsymbol{\Sigma}_x\mathbf{W}^\intercal$。而根据一般的边际分布计算公式有
$$ p(\boldsymbol{y})=\int\mathcal{N}(\boldsymbol{x}|\boldsymbol{\mu}_x,\boldsymbol{\Sigma}_x)\mathcal{N}(\boldsymbol{y}|\mathbf{Wx}+\boldsymbol{b},\boldsymbol{\Sigma}_y)d\boldsymbol{z}  $$
故
$$ \int\mathcal{N}(\boldsymbol{x}|\boldsymbol{\mu}_x,\boldsymbol{\Sigma}_x)\mathcal{N}(\boldsymbol{y}|\mathbf{Wx}+\boldsymbol{b},\boldsymbol{\Sigma}_y)d\boldsymbol{x}=\mathcal{N}(\boldsymbol{y}|\mathbf{W}\boldsymbol{\mu}_x+\boldsymbol{b},\boldsymbol{\Sigma}_y+\mathbf{W}\boldsymbol{\Sigma}_x\mathbf{W}^\mathsf{T}) $$
